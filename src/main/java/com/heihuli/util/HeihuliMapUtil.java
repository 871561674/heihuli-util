...
public class HeihuliMapUtil {
     ...
     */
    public static <T> Map<String, T> listHashMap(List<T> list, String getMethodName) {
        if (null == list) {
            return null;
        }
        ...
        for (T obj : list) {
            if (null != obj) {
                try {
                    map.put(String.valueOf(obj.getClass().getMethod(getMethodName).invoke(obj)), obj);
            ...
            if (!map1.containsKey(key)) {
                ...
                if (map2.get(key) instanceof Map) {
                    if (map1.get(key) instanceof Map) {
                        ...
                        map1.put(key, deepMergeAnyKeyType((Map<K, Object>) map1.get(key), (Map<K, Object>) map2.get(key)));
                    } else {
